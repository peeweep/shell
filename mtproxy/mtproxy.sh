...
————————————————
 ...
 ${Green_font_prefix}10.${Font_color_suffix}  监控 外网IP变更
 ${Green_font_prefix}11.${Font_color_suffix}  定时 更新端口" && echo
	...
	elif [[ "${mtp_modify}" == "11" ]]; then
		Set_crontab_update_port
	else
...
Uninstall_mtproxy(){
	...
	if [[ ${unyn} == [Yy] ]]; then
		...
		if [[ ! -z $(crontab -l | grep -w "mtproxy.sh update") ]]; then
			crontab_update_mtproxy_cron_stop
		fi
		if [[ ! -z $(crontab -l | grep "mtproxy.sh update_port") ]]; then
			crontab_update_port_cron_stop
		fi
		rm -rf "${file}"
		if [[ ${release} = "centos" ]]; then
...
Set_crontab_update_port(){
	check_crontab_installed_status
	crontab_update_port_status=$(crontab -l|grep "mtproxy.sh update_port")
	if [[ -z "${crontab_update_port_status}" ]]; then
		echo && echo -e "当前自动更新端口功能: ${Red_font_prefix}未开启${Font_color_suffix}" && echo
		echo -e "确定要开启 ${Green_font_prefix}MTProxy 自动更新端口${Font_color_suffix} 功能吗？[Y/n]"
		read -e -p "(默认: y):" crontab_update_port_status_ny
		[[ -z "${crontab_update_port_status_ny}" ]] && crontab_update_port_status_ny="y"
		if [[ ${crontab_update_port_status_ny} == [Yy] ]]; then
			crontab_update_port_cron_start
		else
			echo && echo "	已取消..." && echo
		fi
	else
		echo && echo -e "当前自动更新端口功能: ${Green_font_prefix}已开启${Font_color_suffix}" && echo
		echo -e "确定要关闭 ${Red_font_prefix}MTProxy 自动更新端口${Font_color_suffix} 功能吗？[y/N]"
		read -e -p "(默认: n):" crontab_update_port_status_ny
		[[ -z "${crontab_update_port_status_ny}" ]] && crontab_update_port_status_ny="n"
		if [[ ${crontab_update_port_status_ny} == [Yy] ]]; then
			crontab_update_port_cron_stop
		else
			echo && echo "	已取消..." && echo
...
crontab_update_port_cron_start(){
	crontab -l > "$file_1/crontab.bak"
	sed -i "/mtproxy.sh update_port/d" "$file_1/crontab.bak"
	echo -e "\n*/20 * * * * /bin/bash $file_1/mtproxy.sh update_port" >> "$file_1/crontab.bak"
	crontab "$file_1/crontab.bak"
	rm -r "$file_1/crontab.bak"
	cron_config=$(crontab -l | grep "mtproxy.sh update_port")
	if [[ -z ${cron_config} ]]; then
		echo -e "${Error} MTProxy 自动更新端口功能 启动失败 !" && exit 1
	else
		echo -e "${Info} MTProxy 自动更新端口功能 启动成功 !"
	fi
	/bin/bash $file_1/mtproxy.sh update_port
}
crontab_update_port_cron_stop(){
	crontab -l > "$file_1/crontab.bak"
	sed -i "/mtproxy.sh update_port/d" "$file_1/crontab.bak"
	...
	cron_config=$(crontab -l | grep "mtproxy.sh update_port")
	if [[ ! -z ${cron_config} ]]; then
		echo -e "${Error} MTProxy 自动更新端口功能 停止失败 !" && exit 1
	else
		echo -e "${Info} MTProxy 自动更新端口功能 停止成功 !"
	fi
}
crontab_update_port(){
